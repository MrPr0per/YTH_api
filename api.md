...
- /api/v0
	- /debug (должно быть доступно только локально)
		- /addUser (POST) - создает пользователя, возвращает access токен от него (рефреш не устанавливается)
			- доступ:
				- для всех
			- тело:
				- ```js
					{
					  username: str, // должно быть уникальным
					  password: str,
			...
			- ответы:
				- ок - 200 -> `<access_token>`
				- ошибки параметров (username, email, role)
					- 400 - role
					- ~~404~~
					- ~~422~~
				- логические ошибки - 409 - username, email
				- ~~ошибки входа/доступа - 401/403~~
				- ~~ошибки пагинации - 400/404/422~~
				- общие - 429/500
		- /getTokenWithConfirmedEmail (POST) - возвращает токен с доступом `with_confirmed_email`
			- доступ:
				- для всех
			- тело:
				- ```js
					{
						email: str // без валидации
					}
				  ```
			- ответы:
				- ок - 200 -> `<access_token>`
				- ~~ошибки параметров (email)~~
					- ~~400~~
					- ~~404~~
					- ~~422~~
				- ~~логические ошибки - 409~~
	...
	- /auth - логика связанная с авторизацией, представлена в виде контроллеров, а не ресурсов
		- /sendVerificationEmailForRegistration (POST) - отправляет на почту ссылку для регистрации
			...
			- ответы:
				...
				- ошибки параметров (email)
					...
					- ~~404~~
					- ~~422~~
				- логические ошибки - 409 - если почта уже занята
				- ~~ошибки входа/доступа - 401/403~~
				- ~~ошибки пагинации - 400/404/422~~
		...
		- /login (POST) - вход по паролю
			...
			- тело:
				- ```js
					{
						login: str, // логин или емаил
						password: str,
					}
				  ```
			- ответы:
				- ок - 200 -> `{access_token: str}`
				- ошибки параметров (login, password)
					- ~~400~~
					- ~~404~~
					- 422 - неверный логин или пароль (что именно - не сообщается)
				- ~~логические ошибки - 409~~
				- ~~ошибки входа/доступа - 401/403~~
				- ~~ошибки пагинации - 400/404/422~~
				- общие - 404/405/**429**/500 (тут особо строгое ограничение по попыткам входа)
		- /logout (POST) - выходит с устройства, деактивирует refresh и access токен
			...
			- ответы:
				...
				- ~~логические ошибки - 409~~
				- ~~ошибки входа/доступа - 401/403~~
					- ошибок доступа тут быть не может (даже если токены уже истекли, то все равно вернется 204, так просрочить токен это по сути то же самое, что выйти)
				- ~~ошибки пагинации - 400/404/422~~
				- общие - 429/500
		- /refresh (POST) - обновляет refresh и access токены, отзывает старые
			...
			- ответы:
				...
				- ~~ошибки параметров (каких)~~
					...
					- ~~422~~
				- ~~логические ошибки - 409~~
				- ошибки входа/доступа - 401/~~403~~ (401 если истек refresh токен)
				- ~~ошибки пагинации - 400/404/422~~
				- общие - 429/500
		- /changePassword (POST)
			- доступ:
			...
			- ответы:
				- ок - 204
				- ошибки параметров (old_password, new_password)
					- ~~400~~
					- ~~404~~
					- 422 - новый пароль слишком слабый / старый пароль был другим
				- ~~логические ошибки - 409~~
				- ошибки входа/доступа - 401/~~403~~
				- ~~ошибки пагинации - 400/404/422~~
				- общие - 429/500
		- /sendVerificationEmailForResetPassword (POST) - отправляет на почту ссылку для сброса пароля
			...
			- Ответы:
				...
				- ~~ошибки входа/доступа - 401/403~~
				- ~~ошибки пагинации - 400/404/422~~
				- общие - **429**/500 (тут тоже жесткое ограничение для 429)
		- /resetPassword (POST)
			- доступ:
				- `with_confirmed_email`
			- тело:
			...
			- ответы:
				- ок - 204
				- ошибки параметров (new_password)
					- ~~400~~
					- ~~404~~
					- 422 - слабый пароль
				- ~~логические ошибки - 409~~
	...
	- /users
		- /{id} (GET) - получает пользователя без персональных данных
			...
			- ответы:
				...
				- ~~ошибки пагинации - 400/404/422~~
				- общие - 429/500
		- ?{параметры пагинации}&role={...} (GET) - получение всех пользователей
			...
			- доступ: `superadmin`
				...
				- при необходимости ослабим ограничение
			- параметры:
				- `role:str?` - фильтрует по роли, если не указано - не фильтрует
			- ответы:
				- ок - 200 ->`PaginationPage<User>`
				- ошибки параметров (role)
					- 400 - role
					- ~~404~~
					- ~~422~~
				- ~~логические ошибки - 409~~
		...
		- /{id} (PATCH) - редактирование полей
			...
			- ответы:
				...
				- ошибки параметров (id, тело)
					- 400 - id, тело
					- 404 - id
					- 422 - попытка редактирования недоступных полей или если какие то поля невалидны (например username)
				- логические ошибки - 409 - если имя пользователя заменено на уже занятое
				- ошибки входа/доступа - 401/403
				- ~~ошибки пагинации - 400/404/422~~
				- общие - 429/500
		- /{id}/anonymize (POST) - удаляет все данные пользователя, но сохраняет id в связанных ресурсах
			...
			- ответы:
				...
				- ошибки параметров (id)
					...
					- 404 - id
					- ~~422~~
				- логические ошибки - 409 - уже удален
				- ошибки входа/доступа - 401/403
				- ~~ошибки пагинации - 400/404/422~~
				- общие - 429/500
		...
		- / (PUT) - назначает пользователя админом
			...
			- ответы:
				...
				- ошибки параметров (id)
					...
					- 404 - id
					- ~~422~~
				- логические ошибки - 409 - уже админ
				- ошибки входа/доступа - 401/403
				- ~~ошибки пагинации - 400/404/422~~
				- общие - 429/500
		- / (DELETE) - отозвать админку
			...
			- ответы:
				...
				- ошибки параметров (id)
					...
					- 404 - id
					- ~~422~~
				- логические ошибки - 409 - уже не админ
				- ошибки входа/доступа - 401/403
				- ~~ошибки пагинации - 400/404/422~~
				- общие - 429/500
	...
	- /{coursesRegistrations/eventsRegistrations}
		- / (POST) - записывает пользователя на курс/евент
			...
			- ответы:
				- ок - 201 -> `EventRegistration/CourseRegistration`
				- ошибки параметров (тело: user, event)
					- 400 - тело
					- 404 - user, event
					- ~~422~~
				- логические ошибки - 409 - уже есть запись
				- ошибки входа/доступа - 401/403
				- ~~ошибки пагинации - 400/404/422~~
				- общие - 429/500
		- ?{параметры пагинации}&userId={userId}&{courseId/eventId}={{courseId}/{eventId}} (GET)
			...
			- ответы:
				- ок - 200 -> `PaginationPage<EventRegistration/CourseRegistration>`
		...
		- /{id} (GET) - получает запись
			...
			- ответы:
				- ок - 200 -> EventRegistration/CourseRegistration
				- ошибки параметров (id)
					- 400 - id
					- 404 - id
					- ~~422~~
	...
	- /users/{id}/notifications
		- ?{параметры пагинации} (GET) - получить уведомления пользователя
			...
			- параметры:
				- пагинации
			- ответы:
				- ок - 200 -> `PaginationPage<Notification>`
				- ошибки параметров (id)
					- 400 - id
					- 404 - id
					- ~~422~~
		...
		- /{id}/markAsRead (PATCH) - пометить прочитанным
			...
			- ответы:
				...
				- ошибки параметров (userId, notificationId)
					...
					- 404 - userId, notificationId
					- ~~422~~
				- логические ошибки - 409 - уже прочитано
				- ошибки входа/доступа - 401/403
				- ~~ошибки пагинации: 400/404/422~~
				- общие - 429/500
	...
	- /expertApplications
		...
		- / (POST) - создать заявку (и сохранить нужный экшон в истории)
			...
			- ответы:
				- ок - 201 -> `ExpertApplication(Created)`
				- ошибки параметров (тело)
					- 400 - тело
					- ~~404~~
					- ~~422~~
				- логические ошибки - 409 - уже есть заявка, по которой не вынесено решение или если уже эксперт
				- ошибки входа/доступа - 401/~~403~~
				- ~~ошибки пагинации: 400/404/422~~
				- общие - 429/500
		- ?{параметры пагинации}&createdBy={...}&status={...}&acceptedBy={...} (GET) - получить заявки
			...
			- ответы:
				- ок - 200 -> `PaginationPage<ExpertApplication>`
		...
		- /{id} (PATCH) - редактировать заявку
			...
			- ответы:
				...
				- ошибки параметров (id, тело)
					...
					- 404 - id
					- ~~422~~
				- логические ошибки - 409 - если status выше или равен Accepted
				- ошибки входа/доступа - 401/403
				- ~~ошибки пагинации - 400/404/422~~
				- общие - 429/500
