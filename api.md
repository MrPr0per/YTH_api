	...
	- /auth - логика связанная с авторизацией, представлена в виде контроллеров, а не ресурсов
		...
		- /register (POST) - создает пользователя с ролью `student`
			...
			- ответы:
				...
				- ошибки параметров (username, password)
					- 400 - username содержит ==недопустимые (хз какие, надо посмотреть)== символы
					- ~~404~~
					- 422 - пароль слишком слабый (силу можно проверять через `zxcvbn`) или username не удовлетворяет ==условиям (хз каким, надо посмотреть)==
		...
		- /login (POST) - вход по паролю
			...
			- ответы:
				...
				- ошибки параметров (email, password)
					- 400 - email
					- ~~404~~
