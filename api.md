- общая информация для всех запросов:
	...
	- про роли и доступы
		...
		- какие роли будут и какие у них будут доступы:
			...
			- `logged_in` + `with_confirmed_email`
				- смена пароля
	...
	- про пагинацию и фильтрацию
		- у любого запроса списка ресурсов будет одинаковые пагинация и фильтрация:
		- ?cursor={before/after:{id}}&take={count}&orderBy={{fieldName}:asc/desc, ...}&{fieldName}={{value}, ...} (GET) 
			- query параметры пагинации:
				...
				- `orderBy: [{fieldName: str, order: str}]?`
					- поля и направления сортировки
					- по умолчанию сортирует по id
					- сортирует в первую очередь по первому полю, потом по второму и т.д
					- при любой сортировке при одинаковых значениях всех выбранных полей сортирует по id
					- Пример: `?orderby=name:asc,type:desc`
			- query параметры фильтрации:
				- `{fieldName}={{fieldValue},...}` - вернет только те объекты, где значение указанного поля будет принадлежать указанному множеству значений
					- Значения указываются через запятую без пробелов 
						- Например `?id=123,124,125`
					- Допустимо одно значение
						- Например `?id=123`
					- Запятые в значениях недопустимы (экранирования не будет пока оно не понадобится)
		- так же у любого такого запроса будут ответы одного формата
			- ответы:
				- 200
		...
		- ```js
			{
				id: Id, // управляется сервером 
				createdAt: IsoDateTime, // управляется сервером 
				student: Id(User), // управляется сервером 
				message: str,
				status: ExpertApplicationStatus, 
				// ^^^ по умолчанию: NotSent, управляется сервером
				resolution: Id(ExpertApplicationResolution)?, 
				// ^^^ управляется сервером, по умолчанию null
			}
		  ```
	- `ExpertApplicationResolution`
		- ```js
			{
				id: Id, // управляется сервером 
				decidedAt: IsoDateTime, // управляется сервером 
				decidedBy: Id(User), // управляется сервером 
				isApproved: Bool,
				message: str,
			}
		  ```
...
- /api/v0
	...
	- /users/{id}/appointedAdmins
		- ?{параметры пагинации} (GET) - посмотреть кого ты назначал 
			- доступ: `superadmin`
			- ответы:
				- 200 -> `PaginationPage<User>`
				- 400 - при ошибках в параметрах пагинации
				- 403 - нет роли / попытка посмотреть не у себя
				- 404 - нет такого пользователя
	...
	- /registrations
		- / (POST) - записывает пользователя на курс/евент
			- доступ: `student`
			- тело: `Registration` без серверных полей
			- параметры: нет
			- ответы:
				- 201 -> `Registration`
				...
				- 409 - уже записан
				- 422 - семантическая ошибка в теле
		- ?type={activity}&id={id}&{параметры пагинации} (GET) - получает записи на курс/евент/...
			- доступ: для всех
			- %% тело: нет %%
			- параметры (кроме пагинации):
				...
				- course: `Id`
			- ответы:
				- 200 -> `PaginationPage<Registration>`
				- 400 - ошибка формата в id/параметрах
				- 404 - нет такого курса
				- 422 - семантическая ошибка в параметрах
		- ?user={id}&type={activity}&{параметры пагинации} (GET) - получает регистрации пользователя
			- доступ: `student` с нужным id
			- %% тело: нет %%
			- параметры (кроме пагинации):
				- user: `Id`
				- type: `Activity?` 
					- если не указано - вернет все регистрации
			- ответы:
				- 200 -> `PaginationPage<Registration>`
				- 400 - ошибка формата в id/параметрах
	...
	- /expertApplications
		- / (POST) - отправить заявку
			- доступ: `student`
			- %% параметры: нет %%
			- тело: `ExpertApplication`, без серверных полей
			- ответы:
				- 201 -> `ExpertApplication`
				- 400
				- 403 - нет роли
				- 409 - уже есть поданная неодобренная заявка
		- ?{параметры пагинации}&mine={...}?&status={...}&expand={...} (GET) - получить заявки
			- доступ:
				- mine=false => `admin` (чтобы разбирали все отосланные заявки)
				- mine=true => `student` (чтобы посмотреть свои заявки у себя в лк)
			- параметры: 
				- пагинация
				- mine
				- фильтрация по status (если не передан, без фильтрации)
				- expand - если true - раскрывает целиком - и resolution и decidedBy и student
				- ==todo: обобщить фильтрацию на все коллекции==
			- тело: нет 
			- ответы:
				- 201 -> `PaginationPage<ExpertApplication>`
				- 400/403/404/422
		- /{id}&expand={...} (GET) - получить заявку
			- доступ: `admin` или `student`, автор заявки
			- тело: нет
			- параметры:
				- expand - если true - раскрывает целиком - и resolution и decidedBy и student
			- ответы:
		...
		- /{id} (PATCH) - редактировать заявку
			...
			- ответы:  
				...
				- 400/403/404/422
				- 409 - если status выше sent
		- /{id} (DELETE) - удалить заявку
			- доступ: `student`, автор заявки
			- %% параметры/тело: нет %%
			- ответы:  
				- 204  
				- 400/403/404
				- 409 - если status выше sent
		- /{id}/send (POST) - отправить
			- доступ: `student`, автор
			- ответы: 204, 400/403/404/409 - если статус не NotSent
		- /{id}/recall (POST) - отозвать
			- доступ: `student`, автор
			- ответы: 204, 400/403/404/409 - если статус не Sent (т.е. если она и так не отправлена или ее уже взяли на рассмотрение)
		- /{id}/acceptedForReview (POST) - взять на рассмотрение
			- доступ: `admin`
			- ответы: 204, 400/403/404/409 - если статус не Sent (т.е. если ее успели отозвать или забрал на ревью кто то другой)
		- /{id}/cancelReview (POST) - вернуть рассмотрение
			- доступ: `admin`, тот кто взял
			- ответы: 204, 400/403/404/409 - если статус не AcceptedForReview
		- /{id}/completeReview (POST) - завершить рассмотрение, так же отправляет уведомление пользователю о том, что заявка рассмотрена, а так же, если заявка одобрена - делает пользователя экспертом
			- доступ: `admin`, тот кто взял
			- тело: `{resolutionId: Id(ExpertApplicationResolution)}`
			- ответы: 204, 400/403/404/409 - если статус не AcceptedForReview

	- /expertApplicationResolutions
		- / (POST) - создать заключение по заявке
			- доступ: `admin`
			- %% параметры: нет %%
			- тело: ExpertApplicationResolution, без серверных полей
			- ответы:
				- 201 -> `ExpertApplicationResolution`
				- 400
				- 403 - нет роли
				- 404 - заявки нет
				- 409 - у заявки уже есть заключение/заявка скрыта
				- 422
		- /?{параметры пагинации}&mine={isMine} (GET) - посмотреть написанные заключения
			- доступ: `admin`
			- параметры: пагинация, isMine
			- ответы:
				- 201 -> `PaginationPage<ExpertApplication>`
				- 400/403/404/422
		- /{id} - посмотреть заключение
			- доступ: `admin`или тот пользователь, на чью заявку это заключение
			- ответы:
				- 201 -> `ExpertApplicationResolution`
				- 400/403/404
